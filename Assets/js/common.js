...
(function() {
  ...
  function loop() {
    ...
    /* 現在位置をビームエフェクトに公開 */
    window._funnelPositions = funnels
      .filter(f => f.alpha > 0.5)
      .map(f => ({x: f.x, y: f.y}));
...
/* ===== ファンネルビーム＋閃光エフェクト v2 =====
   カーソルの過去位置を標的に、画面遠方から4〜6本のビームが収束。
   各ビームは着弾点まで確実に届く。色はビームごとにバラバラ。
   3〜5秒ごとに自動発動。スマホ除外。
   ========================================== */
(function() {
  if (!window.matchMedia('(hover: hover)').matches) return;
  ...
  const cv = document.createElement('canvas');
  cv.id = 'beam-canvas';
  cv.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:9997;';
  document.body.appendChild(cv);
  const ctx = cv.getContext('2d');
  let W = cv.width = window.innerWidth;
  let H = cv.height = window.innerHeight;
  ...
  let beams = [], flashes = [];
  let attackTimer = 0;
  let attackInterval = 180 + Math.floor(Math.random() * 120);

  // カーソル位置の履歴（約1秒前の位置を標的にする）
  let cursorHistory = [];
  ...
  // 各ビームの色定義（バラバラ）
  const BEAM_COLORS = [
    { core:'rgba(255,245,250,', glow:'rgba(255,160,210,' },  // 白ピンク
    { core:'rgba(200,235,255,', glow:'rgba(80,170,255,'  },  // 青白
    { core:'rgba(255,252,210,', glow:'rgba(255,220,80,'  },  // 黄白
    { core:'rgba(230,255,250,', glow:'rgba(80,220,180,'  },  // 青緑白
    { core:'rgba(255,220,255,', glow:'rgba(200,80,255,'  },  // 紫白
    { core:'rgba(255,255,255,', glow:'rgba(180,220,255,' },  // 純白青
  ];

  // 発射元：画面の遠い端（画面外周から少し外側）
  function remoteSrc(tgt) {
    // 着弾点から遠ざかる方向に画面外まで延ばす
    const angle = Math.random() * Math.PI * 2;
    // 画面対角線長さ以上の距離
    const dist = Math.max(W, H) * (0.9 + Math.random() * 0.4);
    let sx = tgt.x + Math.cos(angle) * dist;
    let sy = tgt.y + Math.sin(angle) * dist;
    return { x: sx, y: sy };
  }

  function launchAttack(tgt) {
    const n = 4 + Math.floor(Math.random() * 3);
    // 色をシャッフルしてn個選ぶ
    const shuffled = [...BEAM_COLORS].sort(() => Math.random() - 0.5);
    let maxDelay = 0;

    for (let i = 0; i < n; i++) {
      const src = remoteSrc(tgt);
      const delay = Math.floor(Math.random() * 20);
      maxDelay = Math.max(maxDelay, delay);
      const col = shuffled[i % shuffled.length];

      beams.push({
        ox: src.x, oy: src.y,
        tx: tgt.x, ty: tgt.y,  // 着弾点まで確実に届く
        life: 1.0,
        decay: 0.048 + Math.random() * 0.022,
        width: 0.2 + Math.random() * 0.25,  // 極細：0.2〜0.45px
        col, delay,
      });
    }
    setTimeout(() => spawnFlash(tgt.x, tgt.y), (maxDelay + 12) * 16);
  }
  ...
  function spawnFlash(x, y) {
    flashes.push({
      ...
      rays: Array.from({length: 8}, (_, i) => ({
        angle: i/8*Math.PI*2 + Math.random()*.5,
        len:   10 + Math.random()*20,
        width: 0.3 + Math.random()*.4,
      })),
      rings: [
        { r:0, maxR:18, speed:2.0 },
        { r:0, maxR:30, speed:2.8 },
      ],
    });
  }
  ...
  function updateBeam(b) {
    ...
    ctx.strokeStyle = b.col.glow + '0.9)';
    ctx.lineWidth = b.width * 4;
    ctx.shadowColor = b.col.glow + '1.0)';
    ctx.shadowBlur = 5;
    ctx.lineCap = 'round';
    ...
    ctx.strokeStyle = b.col.core + '1.0)';
    ctx.lineWidth = b.width;
    ctx.shadowColor = b.col.glow + '0.8)';
    ctx.shadowBlur = 2;
    ctx.lineCap = 'round';
    ctx.beginPath(); ctx.moveTo(b.ox, b.oy); ctx.lineTo(b.tx, b.ty); ctx.stroke();
    ctx.restore();
  ...
  function updateFlash(f) {
    f.timer++;
    const a = Math.max(0, 1 - f.timer / 16);

    // 中心白光
    ctx.save();
    ctx.globalAlpha = a * .95;
    ctx.fillStyle = 'rgba(255,255,255,0.98)';
    ctx.shadowColor = 'rgba(255,180,220,1.0)';
    ctx.shadowBlur = 18;
    ctx.beginPath(); ctx.arc(f.x, f.y, 2 + f.timer*.6, 0, Math.PI*2); ctx.fill();
    ctx.restore();
    ...
    ctx.shadowColor = 'rgba(255,80,180,1.0)';
    ctx.shadowBlur = 20;
    ctx.beginPath(); ctx.arc(f.x, f.y, 4 + f.timer*1.1, 0, Math.PI*2); ctx.fill();
    ctx.restore();
    ...
    f.rays.forEach(r => {
      ...
      ctx.shadowColor = 'rgba(255,160,210,0.8)';
      ctx.shadowBlur = 4;
      ctx.lineCap = 'round';
      const ex = f.x + Math.cos(r.angle)*r.len*(.3+f.timer*.12);
      const ey = f.y + Math.sin(r.angle)*r.len*(.3+f.timer*.12);
    ...
    f.rings.forEach(r => {
      r.r += r.speed;
      const rl = Math.max(0, 1 - r.r/r.maxR);
      if (rl <= 0) return;
      ctx.save();
      ctx.globalAlpha = rl * .55;
      ctx.strokeStyle = 'rgba(255,160,220,0.9)';
      ctx.lineWidth = 0.45;
      ctx.shadowColor = 'rgba(255,80,180,0.7)';
      ctx.shadowBlur = 4;
      ctx.beginPath(); ctx.arc(f.x, f.y, r.r, 0, Math.PI*2); ctx.stroke();
      ctx.restore();
    });
  ...
  function loop() {
    ...
    // カーソル履歴を記録（約60フレーム≒1秒前の位置を保持）
    cursorHistory.push({ x: curX, y: curY });
    if (cursorHistory.length > 65) cursorHistory.shift();

    attackTimer++;
    if (attackTimer >= attackInterval) {
      // 約1秒前のカーソル位置を標的に
      const pastPos = cursorHistory[0] || { x: curX, y: curY };
      launchAttack(pastPos);
      attackTimer = 0;
      attackInterval = 160 + Math.floor(Math.random() * 140);
    }

    beams = beams.filter(b => updateBeam(b));
    flashes = flashes.filter(f => updateFlash(f));
    requestAnimationFrame(loop);
  }
  loop();
})();
...
/* ===== 共通サイドバー自動生成 =====
   各ページの <aside class="sidebar" id="sidebar"></aside> に対して
   renderSidebar({ root:'../../', active:'step01' }) を呼ぶだけで
   サイドバー全体を挿入します。

   オプション：
     root   : このページからサイトルートへの相対パス
     active : アクティブにするセクション識別子
               'home' / 'ai-news' / 'step01' / 'step02' /
               'emoji' / 'exams' / 'construction'
     sbLabel: サイドバー下部の大きい数字ラベル（省略可）
     sbVal  : サイドバー下部の値（省略可）
     sbSub  : サイドバー下部の小テキスト（省略可）
*/
function renderSidebar(opts = {}) {
  ...
  const sbLabel = opts.sbLabel ?? 'OZ²';
  const sbVal   = opts.sbVal   ?? 'AI';
  const sbSub   = opts.sbSub   ?? 'QUBELEY';

  function a(href, cls, icon, label, extra=''){
    const isActive = active && href.includes(active) ? ' active' : '';
    return `<a href="${href}" class="nav-item${cls}${isActive}"><span class="nav-icon">${icon}</span>${label}${extra}</a>`;
  }
  const SOON = '<span class="nav-coming">SOON</span>';

  el.innerHTML = `
    <nav class="sb-nav">
      ${a(root+'index.html', '', '⊞', 'ホーム')}
      ${a(root+'pages/ai-news/index.html', active==='ai-news'?' active':'', '📡', 'AI最新情報')}
      <div class="nav-accordion open" id="accordionRoadmap">
        <div class="nav-accordion-header nav-item" onclick="toggleAccordion('accordionRoadmap')">
          <span class="nav-icon">🗺</span>AI学習ロードマップ<span class="nav-accordion-arrow">▸</span>
        </div>
        <div class="nav-accordion-body">
          <a href="${root}pages/step01-start/index.html" class="nav-item nav-sub${active==='step01'?' active':''}"><span class="nav-step-badge">01</span>まず触ってみる</a>
          <a href="${root}pages/construction.html?section=うまく伝える" class="nav-item nav-sub"><span class="nav-step-badge">02</span>うまく伝える<span class="nav-coming">SOON</span></a>
          <a href="${root}pages/construction.html?section=安全に使う" class="nav-item nav-sub"><span class="nav-step-badge">03</span>安全に使う${SOON}</a>
          <a href="${root}pages/construction.html?section=ツールを知る" class="nav-item nav-sub"><span class="nav-step-badge">04</span>ツールを知る${SOON}</a>
          <a href="${root}pages/construction.html?section=仕事で使う" class="nav-item nav-sub"><span class="nav-step-badge">05</span>仕事で使う${SOON}</a>
        </div>
      </div>
      <div class="nav-accordion" id="accordionKnowledge">
        <div class="nav-accordion-header nav-item" onclick="toggleAccordion('accordionKnowledge')">
          <span class="nav-icon">🧠</span>Knowledge<span class="nav-accordion-arrow">▸</span>
        </div>
        <div class="nav-accordion-body">
          <a href="${root}pages/knowledge/emoji.html" class="nav-item nav-sub${active==='emoji'?' active':''}"><span class="nav-step-badge">—</span>絵文字集</a>
        </div>
      </div>
      <div class="nav-accordion" id="accordionExams">
        <div class="nav-accordion-header nav-item" onclick="toggleAccordion('accordionExams')">
          <span class="nav-icon">📝</span>試験攻略<span class="nav-accordion-arrow">▸</span>
        </div>
        <div class="nav-accordion-body">
          <a href="${root}pages/exams/aias/aias_quiz.html" class="nav-item nav-sub${active==='exams-aias'?' active':''}"><span class="nav-step-badge">—</span>AIエージェントST</a>
        </div>
      </div>
    </nav>
    <div class="sb-footer">
      <div class="license-box">
        <div class="lic-label">${sbLabel}</div>
        <div class="lic-val" style="font-size:1.4rem;">${sbVal}</div>
        <div class="lic-sub">${sbSub}</div>
      </div>
    </div>
  `;
  ...
  if(hb) hb.addEventListener('click', () => _open(!(sb && sb.classList.contains('open'))));
  if(bd) bd.addEventListener('click', () => _open(false));
  document.querySelectorAll('.sidebar .nav-item').forEach(e => {
    e.addEventListener('click', () => _open(false));
  });
}
